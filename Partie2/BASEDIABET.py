...
BASEDIABET = pd.read_csv("Partie2/LOCAL_preprocessed.csv")

# === 2️⃣ PRÉPARATION DES DONNÉES BASEDIABET ===
print("🔧 Préparation des données BASEDIABET...")
X_BASEDIABET = BASEDIABET.drop(columns=['label', 'Date'])
y_BASEDIABET = BASEDIABET['label']

# Division train/test
X_train_BASEDIABET, X_test_BASEDIABET, y_train_BASEDIABET, y_test_BASEDIABET = train_test_split(
    X_BASEDIABET, y_BASEDIABET, test_size=0.3, random_state=42, stratify=y_BASEDIABET
)

print(f"Distribution BASEDIABET - Train: {np.bincount(y_train_BASEDIABET)}, Test: {np.bincount(y_test_BASEDIABET)}")

# === 3️⃣ DÉFINITION DES MODÈLES ET MÉTHODES (AVEC SVM ET NAIVE BAYES) ===
models = {
    'XGBoost': XGBClassifier(
        ...
        max_depth=6,
        random_state=42,
        scale_pos_weight=np.sum(y_train_BASEDIABET == 0) / np.sum(y_train_BASEDIABET == 1)
    ),
    ...
    'SVM': SVC(
        ...
        C=1.0,
        gamma='scale',
        probability=True,  # Important pour ROC curve
        random_state=42
    ),
...
for sample_name, sampler in sampling_methods.items():
    ...
    if sampler:
        X_resampled, y_resampled = sampler.fit_resample(X_train_BASEDIABET, y_train_BASEDIABET)
        print(f"Distribution après {sample_name}: {np.bincount(y_resampled)}")
    else:
        X_resampled, y_resampled = X_train_BASEDIABET, y_train_BASEDIABET
    ...
    for model_name, model in models.items():
        ...
        try:
            ...
            # Prédictions
            y_pred = model.predict(X_test_BASEDIABET)
            y_proba = model.predict_proba(X_test_BASEDIABET)[:, 1]
            
            # Métriques
            report = classification_report(y_test_BASEDIABET, y_pred, output_dict=True)
            auc = roc_auc_score(y_test_BASEDIABET, y_proba)
...
plt.tight_layout()
plt.savefig('Partie2/BASEDIABET_results/comparaison_performances.png', dpi=300, bbox_inches='tight')
...
for sample_name in sampling_methods.keys():
    for model_name in models.keys():
        if results[sample_name][model_name] is not None:
            ...
            fpr, tpr, _ = roc_curve(y_test_BASEDIABET, y_proba)
...
plt.tight_layout()
plt.savefig('Partie2/BASEDIABET_results/courbes_roc.png', dpi=300, bbox_inches='tight')

# === 5.3 MATRICES DE CONFUSION ===
# Ajuster la taille pour 4 modèles
...
for i, sample_name in enumerate(sampling_methods.keys()):
    for j, model_name in enumerate(models.keys()):
        ...
        if results[sample_name][model_name] is not None:
            ...
            cm = confusion_matrix(y_test_BASEDIABET, y_pred)
...
plt.tight_layout()
plt.savefig('Partie2/BASEDIABET_results/matrices_confusion.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Partie2/BASEDIABET_results/precision_rappel_par_classe.png', dpi=300, bbox_inches='tight')
...
df_performance.to_csv('Partie2/BASEDIABET_results/tableau_performances.csv', index=False, encoding='utf-8-sig')
...
print("\n✅ Toutes les visualisations ont été sauvegardées dans le dossier 'Partie2/BASEDIABET_results/'")
